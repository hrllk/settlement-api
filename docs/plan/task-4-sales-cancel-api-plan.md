...
7. **커맨드 포트 `SaleCommandPort`와 그 어댑터를 Task 4가 소유한다.**
   ...
   Task 2의 `SettlementDataPort`는 조회 메서드 4개뿐이라 등록 경로를 덮지 못한다. 포트의 모양은 유스케이스가 무엇을 필요로 하는지에서 나오므로, 그것을 아는 Task 4가 선언과 구현을 함께 갖는다. 어댑터는 Task 2가 만든 Spring Data 리포지토리를 감싼다.
