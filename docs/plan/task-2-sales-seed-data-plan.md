...
`SettlementDataPort`의 JPA 구현체와, Task 3 기준표를 그대로 재현하는 초기 데이터를 만든다. 계산 규칙과 API는 범위 밖이다.
...
- Task 3: `SettlementDataPort` 인터페이스, `SaleData` / `CancelData` 값 타입
...
- Spring Data 리포지토리 4종과 `SettlementDataPort` JPA 어댑터 (조회 전용)
- `data.sql` 초기 데이터
- 시드 재현성 테스트
...
| 2.5 | `SettlementDataPort` JPA 어댑터 | 4개 메서드 구현. 엔티티 → 값 타입 변환. `null` 반환 금지, 빈 컬렉션 입력 방어. **Task 3 포트 확정 후 착수** |
