...
**15번 하나로 충분한 이유는 전역 처리기가 하나이기 때문이다.** 오류 응답의 모양을 결정하는 코드 경로가 `GlobalExceptionHandler`의 헬퍼 한 곳이다. 거기서 나온 응답 하나를 통째로 잠그면 나머지 9건은 `code`만 봐도 된다. 같은 `contentType` 단언을 10번 복사하면 포맷을 바꿀 때 10곳을 고치게 된다.

**기존 단언은 하나도 안 깨진다.** `$.code`는 확장 멤버가 최상위로 직렬화되어 유지되고 `$.status`도 RFC 9457의 표준 필드다. `$.message`를 보는 단언은 애초에 0건이라 `detail`로 바뀌어도 영향이 없다.
