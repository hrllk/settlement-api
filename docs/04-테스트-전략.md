...
**테스트 109건.** 규칙을 소유한 Task가 그 규칙의 테스트도 소유합니다.
...
| Task 3 | 44 | 계산 규칙 전부. Spring·H2 없이 돕니다 |
| Task 4 | 35 | 등록·취소 API, 애그리게이트 불변식, 오류 포맷, 인가 판정 |
| Task 5 | 12 | 정산 조회 API, 접근 경계, opt-in 가드 |
| Task 6 | 3 | HTTP 통합 1건, 결정성 가드 2건 |
| **합계** | **109** | |

**Task 3이 44건으로 가장 큽니다.** 계산 규칙 전부를 Spring도 H2도 없이 잠급니다. 그래서
나머지 Task가 같은 숫자를 다시 단언하지 않아도 됩니다.
...
| 두 번 연속 실행 | `./gradlew cleanTest test` × 2 | 109건 / 109건, 동일 |
...
테스트가 **존재하는지**만 봅니다. 그 테스트가 **옳은지**는 보지 못합니다. 잘못된 기대값을
박아둔 테스트도 이 표에서는 초록색입니다. 그 층은 아래 실행 검증이 맡습니다.

## 실행 검증

로컬에서 되는 것과 평가자 환경에서 되는 것은 다릅니다. 아래는 **실제로 돌린 결과**입니다.
검수에서 나온 결함을 고친 뒤 숫자만 고치지 않고 처음부터 다시 실행했습니다.

| 항목 | 결과 |
| --- | --- |
| 새로 클론한 곳에서 `./gradlew clean test` | **109건 / 실패 0** |
...
점검에서 결함이 나오면 **여기서 고치지 않고** 해당 Task로 되돌립니다. 제출 직전에 급하게
고친 코드가 테스트를 거치지 않고 들어가는 것이 가장 위험합니다.
