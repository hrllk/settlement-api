...
| --- | --- | ---: | --- |
| cancel-1 | sale-3 | 80,000 | `2025-03-25T10:00:00+09:00` |
| cancel-2 | sale-4 | 30,000 | `2025-03-26T10:00:00+09:00` |
| cancel-3 | sale-5 | 60,000 | `2025-02-03T10:00:00+09:00` |

판매 7건은 원본 과제 샘플이다. 취소 3건은 원본에 없어 직접 정의했다 — 금액과 귀속 월은 `tasks.json`이 확정했다. **시각은 Task 2가 소유한다** — 세 건 모두 10:00 KST이며 Task 2 시드와 같은 값을 쓴다. 월 경계에서 멀어 어떤 구간 구현에서도 귀속 월이 흔들리지 않으므로 경계 검증(3.1)과 집계 검증이 분리된다.
...
Task 6은 이 단언을 반복하지 않는다. **규칙을 소유한 Task가 테스트도 소유한다.** 누적 초과 환불 거부는 Task 4, 운영자 목록 조립과 전체 합계는 Task 5가 각각 단언한다. Task 6은 아무도 혼자 못 보는 것만 맡는다 — HTTP 통합 흐름 1건과 커버리지 감사다.
